...
- [~] Apply the migrations to the target Supabase database and regenerate `types/supabase.ts` (2026-03-14 11:15 - the follow-up location-access migration was added and local typings were extended for active slices, but `types/supabase.ts` still contains placeholder table definitions and no full regeneration has been run)
